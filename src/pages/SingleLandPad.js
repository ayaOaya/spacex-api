...
export default function SingleLandPad() {
  ...
  return (
    <>
   {!singleLandPad ? <Loading /> :  <section className="py-32 max-width flex flex-col-reverse gap-10 md:grid md:grid-cols-2">
      <article>
        <h1 className="heading">{singleLandPad.full_name}</h1>
        <h2 className="text-white font-bold mb-10 text-lg opacity-80 lg:text-2xl mt-2">{singleLandPad.name}</h2>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 mt-10">
          ...
          <li>{singleLandPad.launches.length} launches</li>
            ...
            {singleLandPad.status === "active" ? <li className="text-emerald-500 capitalize">{singleLandPad.status}</li> : <li className="text-rose-500 capitalize">{singleLandPad.status}</li>}
          </ul>
        ...
        <p className="text-white opacity-75 mt-10">{singleLandPad.details}</p>

        {/*noreferrer is used to prevent cross site scripting */}
        ...
        <li><Link to="/landpads" className="text-white text-sm opacity-75 hover:opacity-100">&larr; Back</Link></li> 
        </ul>

      </article>

      <article>
        <img src={singleLandPad.images.large[0]} alt={singleLandPad.full_name} className="h-full"></img>
      </article>
